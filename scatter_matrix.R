...
cv_glasso <- function(Y, lambda_grid, K = 5, m = 3, method = c("SAMPLE", "SPCA", "TylerSPCA", "FLW","IPSN","RegTME"),normalized = FALSE) {
  n <- nrow(Y)
  ...
  for (k in 1:K) {
    ...
    if (!normalized) {
      Sigma_train_u <- est_train$Sigma_u_hat
      Sigma_valid_u <- est_valid$Sigma_u_hat
    } else {
...
loss_for_sglasso_nonsparse <- function(Y, Sigma_true, Sigma_u_true,
...
main_sglasso_loss <- function(p = 100,
                              sims = 10
) {
  ...
  for (s in 1:sims) {
    ...
    for (method in methods) {
      ...
      loss <- loss_for_sglasso_nonsparse(Y, Sigma_true, Sigma_u, method = method, estimate_2_part = estimate_2_part)
...
set.seed(988)
sglasso_results <- main_sglasso_loss(p = 500, sims = 100)
methods <- c("SAMPLE", "SPCA", "TylerSPCA", "FLW", "IPSN","RegTME")
for (method in methods) {
  cat("=== 方法:", method, "===\n")
  mean_vals <- sglasso_results$mean_results[[method]]
  sd_vals <- sglasso_results$sd_results[[method]]
