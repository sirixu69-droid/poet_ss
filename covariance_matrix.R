...
main_sglasso_loss_onecase <- function(p = 100,
                                      sims = 10,
                                      mi = 1,
                                      n = 250,
                                      m = 3) {
  ...
  for (s in 1:sims) {
    ...
    for (method in methods) {
      loss <- loss_for_sglasso_nonsparse(Y, Sigma_true, Sigma_u,
                                         method = method)
...
for (ell in seq_along(mi_vec)) {
  mi <- mi_vec[ell]
  sglasso_results <- main_sglasso_loss_onecase(p = 500, sims = 100, mi = mi)
  mean_mat <- matrix(NA_real_, nrow = M, ncol = K,
                     dimnames = list(methods, metrics))
  sd_mat   <- matrix(NA_real_, nrow = M, ncol = K,
                     dimnames = list(methods, metrics))
  for (meth in methods) {
    mean_vals <- sglasso_results$mean_results[[meth]]
    sd_vals   <- sglasso_results$sd_results[[meth]]
    for (met in metrics) {
